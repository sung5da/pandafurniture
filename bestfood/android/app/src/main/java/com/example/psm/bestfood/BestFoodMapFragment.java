...
import com.example.psm.bestfood.adapter.MapListAdapter;
import com.example.psm.bestfood.item.FoodInfoItem;
import com.example.psm.bestfood.item.GeoItem;
...
public class BestFoodMapFragment extends Fragment
     ...
     * @param inflater XML을 객체로 변환하는 LayoutInflater 객체
    ...
    /**
     * onCreateView() 메소드 뒤에 호출되며 구글 맵을 화면에 보여준다.
    ...
    /**
     * 구글 맵이 준비되었을 때 호출되며 구글 맵을 설정하고 기본 마커를 추가하는 작업을 한다
     * @param map 구글 맵 객체
     */
    @Override
    ...
    /**
     * 구글 맵의 카메라를 위도와 경도 그리고 줌레벨을 기반으로 이동
    ...
    /**
     * 맛집리스트를 지도에 표시하는 메소드를 호출하고 지도에서 원을 그린다
     * @param list 맛집리스트     */
    ...
    /**
     * 지도를 일정 레벨 이상 확대했을 경우, 해당 위치에 있는 맛집리스트를 서버에 요청한다.
     */
